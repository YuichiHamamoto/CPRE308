...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	//find the least time of arribal
...
int PRAlgo_LRU(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	int frame_to_evict = 0;
	/*TODO: fill in the code below */
	int least_time_of_acc = PageFrames[0].time_of_access;
	int i;

	//find the least time of access
	for (i = 1; i < num_frames; i++){
		if (PageFrames[i].time_of_access < least_time_of_acc){
      		least_time_of_acc = PageFrames[i].time_of_access;
      		frame_to_evict = i;
    }
  }
  return frame_to_evict;
...
int PRAlgo_OPT(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int i;
	int j;
	int found[num_frames];
	int done;

	//set all to 0, "not found yet" 
	for(i= 0;i<num_frames;i++){
		found[i]=0;
	}
	...
	for(i= current_access;i<num_accesses;i++){
		...
		for(j=0;j<num_frames;j++){
			//the page frame has not been processed yet
			if(found[j]==0){
				done = 0;
		...
		//if all of them has been processed
		if(done){
			break;
		}
	}
