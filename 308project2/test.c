...
/* max line length for lines in the output file */
...
int wait_time_initial = 60;
int wait_time_final = 120;
...
void analyzeOutputFile(int *expected_balances, int *expected_isf_req_ids, int num_trans_initial, int num_trans_random) {
	...
	while(fgets(line, LINE_LENGTH, out) != NULL) {
		lineNumber++;
	...
	if (!req_answered[i])
		missing_req_ids[count_missing++] = i + 1;
	if (count_missing > 0) {
